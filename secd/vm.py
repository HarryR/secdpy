...
iscons = lambda X: isinstance(X, (list, tuple))
isbool = lambda X: isinstance(X, bool)
isnumb = lambda X: isinstance(X, int)
issym = lambda X: isinstance(X, str)
isatom = lambda X: any([issym(X), isnumb(X), isbool(X)])
...
COMMANDS.update({
	...
	#
	#	if issymb(car(s)) or isnumb(car(s))
	#		then s := cons(t, cdr(s))
	#		else s := cons(f, cdr(s))
	#	c = cdr(c)
	#
...
ALIASES = {'RTN': 'RET', 'REM': 'MOD', 'LEQ': 'LE', 'GEQ': 'GE', 'IF': 'SEL',
		   'SHL': 'LSHIFT', 'SHR': 'RSHIFT',
			...
			21: 'STOP'}
COMMANDS.update({x: COMMANDS[y] for x, y in ALIASES.items()})
